...
describe('WalletService', () => {
  ...
  describe('rechargeWallet', () => {
    it('should recharge the wallet if client is found and phone matches', async () => {
      ...
      const result = await service.rechargeWallet(data);

      expect(result).toEqual(wallet);
      expect(prismaService.client.findUnique).toHaveBeenCalledWith({ where: { document: data.document } });
    ...
    it('should throw an error if client is not found', async () => {
      ...
      await expect(service.rechargeWallet(data)).rejects.toThrow('Client not found or phone number does not match');
    });
  ...
  describe('getBalance', () => {
    it('should return the balance if client is found and phone matches', async () => {
      const document = '123';
      ...
      const result = await service.getBalance(document, phone);

      expect(result).toEqual(wallet.balance);
      expect(prismaService.client.findUnique).toHaveBeenCalledWith({ where: { document } });
      expect(prismaService.wallet.findUnique).toHaveBeenCalledWith({ where: { clientId: client.id } });
    });
    ...
    it('should throw an error if client is not found', async () => {
      ...
      await expect(service.getBalance(document, phone)).rejects.toThrow('Client not found or phone number does not match');
    });
  ...
  describe('createWallet', () => {
    it('should create a wallet if client does not have one', async () => {
      ...
      expect(result).toEqual(wallet);
      expect(prismaService.wallet.findUnique).toHaveBeenCalledWith({ where: { clientId } });
    ...
    it('should throw an error if wallet already exists for the client', async () => {
      ...
      await expect(service.createWallet(clientId)).rejects.toThrow('Wallet already exists for this client');
      expect(prismaService.wallet.findUnique).toHaveBeenCalledWith({ where: { clientId } });
